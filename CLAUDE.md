...
- `docs/design.md` — **what the game is, and how it works.** Both standard templates in
  one file, broadest first. Part I is the concept: title, concept statement, pillars,
  genre, audience, selling points, style, platform, scope. Part II is the detailed GDD:
  status, key moments, progression, world, UI, systems, game objects, tools, technical
  risks — present tense and real numbers, or an explicit `?`. Part I is one page; if
  Part I is growing, say so. Part II grows as the game gets built.
...
- `docs/inside/` — **researched background on how INSIDE was built.** One file per
  topic; `README.md` says which to open for what. Reference, not requirements. Lines
  are tagged `[doc]` (stated by Playdead), `[obs]` (observable), `[guess]` (inference).
  Never treat a `[guess]` as settled, and never treat any of it as a requirement — most
  of it was built by a funded studio of 25 over six years. Nothing here is a task until
  it's a work item in Plane.
...
- `reference/transcripts/` — raw auto-generated transcripts of Playdead talks;
  ...
  `docs/inside/`. Each file opens with a frontmatter block naming the source.
  Third-party source material, not project decisions. Read on request or when
  verifying a claim in `docs/inside/`. Auto-captions contain transcription
